# BE-P4-002: Shell Command Executor
...
- Develop as standalone module, test with safe commands (`echo`, `ls`)
- No dependency on plan engine — accept `{ command, cwd }` directly
- Export: `runCommand()` with streaming callback
- Executor (BE-P4-004) will mock this with canned output until integration
