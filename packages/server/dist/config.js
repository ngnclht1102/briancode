...
export function loadConfig(cliOverrides) {
    ...
    if (globalConfig) {
        if (globalConfig.defaultProvider)
            config.defaultProvider = globalConfig.defaultProvider;
    ...
    if (cliOverrides?.provider)
        config.defaultProvider = cliOverrides.provider;
    if (cliOverrides?.model) {
        const prov = config.providers[config.defaultProvider];
        if (prov)
            prov.model = cliOverrides.model;
    }
    cachedConfig = config;
...
export function getRecentProjects() {
    ensureConfigDir();
    const raw = readJsonFile(GLOBAL_CONFIG_FILE);
    return raw?.recentProjects ?? [];
}
export function addRecentProject(projectPath) {
    ensureConfigDir();
    const existing = (readJsonFile(GLOBAL_CONFIG_FILE) ?? {});
    const list = existing.recentProjects ?? [];
    const name = path.basename(projectPath);
    const entry = { path: projectPath, name, lastOpened: new Date().toISOString() };
