...
export function readFile(relativePath, startLine, endLine) {
    const absPath = resolveProjectPath(relativePath);
    if (!absPath)
        return `Error: Path traversal blocked for "${relativePath}"`;
    ...
    for (let i = 0; i < bytesRead; i++) {
        if (sample[i] === 0)
            return `[Binary file, ${stat.size} bytes]`;
    }
...
export function readFileRaw(relativePath, maxBytes = MAX_RAW_BYTES) {
    const absPath = resolveProjectPath(relativePath);
    if (!absPath)
        return `Error: Path traversal blocked for "${relativePath}"`;
    if (!fs.existsSync(absPath))
